...
2. Prepare `776bc/proposals/<descriptive-slug>/index.html` and supporting relative assets. Add an All proposals link. Retain any applicable access controls; noindex does not make a URL private.
3. Update `776bc/proposals/index.html` with every existing proposal plus the new one. Preserve existing links. Use proposal title, client, date, short description and confirmed commercial summary; do not invent acceptance status. The index must not redirect to a single proposal.
...
776BC proposals moved to `/776bc/proposals/` on 7 September 2026. Keep the old `/proposals/` index and individual URLs as forwarding pages so shared links continue to work. Update navigation and printed URLs. A path-only move does not change proposal terms or version: preserve approval storage keys and identifiers to retain existing browser records on the same origin.
